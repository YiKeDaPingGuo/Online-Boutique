...
# 子图1: F1 趋势
exp_names = ['#1\n基线', '#2\n降维', '#3\n强故障(短)', '#4\n服务聚合', '#5\n精简特征']
f1_scores = [0.112, 0.109, 0.553, 0.496, 0.589]
precision_scores = [0.059, 0.058, 0.383, 0.330, 0.457]
recall_scores = [1.0, 1.0, 1.0, 1.0, 0.832]

colors = ['#d4d4d4', '#d4d4d4', '#a8d8ea', '#7ec8e3', '#3b8ed0']
...
axes[0].set_title('F1 变化趋势 (5组对照实验)', fontsize=13, fontweight='bold')
...
print('图1: results_comparison.png')
...
ax.set_title('异常分数分布: 正常 vs 异常 (实验#5)', fontsize=13, fontweight='bold')
ax.legend(fontsize=10)

plt.tight_layout()
plt.savefig('anomaly_score_distribution.png', dpi=150, bbox_inches='tight')
print('图2: anomaly_score_distribution.png')

# ═════════════════════════════════════════════
# 图3: 维度 vs F1 关系
# ═════════════════════════════════════════════
fig, ax = plt.subplots(figsize=(7, 4))
dims = [494, 234, 234, 156, 36]
labels = ['#1', '#2', '#3', '#4', '#5']
...
plt.tight_layout()
plt.savefig('dimension_vs_f1.png', dpi=150, bbox_inches='tight')
print('图3: dimension_vs_f1.png')
print('\n三张图全部生成完成!')
